...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    return max(0, 1 - np.matmul(label*theta, feature_vector) - theta_0)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    hinge_loss = 0.
    for r in range(0, n):
       hinge_loss += hinge_loss_single(feature_matrix[r], labels[r], theta, theta_0)
...
def pegasos_single_step_update(
    ...
    if label * (np.matmul(feature_vector, current_theta) + current_theta_0) <= 1:
        ...
        #theta_0 = ( 1 - L * eta) * current_theta_0
        theta_0 = current_theta_0
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(n):
            eta_t += 1
            theta, theta_0 = pegasos_single_step_update(feature_matrix[i], labels[i], L, 1/np.sqrt(float(eta_t)), theta, theta_0) 
    return (theta, theta_0)
#pragma: coderesponse end
